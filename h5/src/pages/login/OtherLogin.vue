<template>
  <div class="other-login" id="other-login">
    ...
    <div class="content">
      ...
      <from-bottom-dialog page-id="other-login" v-model="isOtherLogin" :show-heng-gang="false" height="270rem"
        mode="white">
...
const isOtherLogin = ref(false);
const otherLogin = async () => {
  const verifyRes = protocolRef.value!.verify();
  if (!verifyRes) return;
  isOtherLogin.value = true;
}
...
onActivated(() => {
  if (userStore.userInfo) {
    ...
    route.meta.leaveTransition = 'down';
    router.back();
  }
})
</script>
